...
{\Large Isolating Fault-Inducing Inputs to SDN Control Software}}

\author{Colin Scott\thanks{UC Berkeley}     %$^\dagger$
\and Andreas Wundsam\thanks{Big Switch Networks} %$^\diamond$
\and Sam Whitlock\thanks{ICSI}  % $^*$
\and Andrew Or\footnotemark[1] % $^\dagger$
\and Eugene Huang\footnotemark[1] %$^\dagger$
\and Kyriakos Zarifis\thanks{USC} %$^\ddagger$
\and Scott Shenker\footnotemark[3]~\footnotemark[1] % $^\dagger$
%\and {\begin{tabular}{cccc}$^\dagger${\large\it UC Berkeley} &
%$^\diamond${\large\it Big Switch Networks} & $^*${\large\it ISCI} & $^\square${\large\it USC}}
}
