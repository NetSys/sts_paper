Based on anecdotal evidence from colleagues and acquaintances in the industry,
it seems clear that
developers of SDN control software spend much of their time
...
And while the purpose of SDN control software is to simplify network
management for operators, the software itself is a highly complex system that may involve
distributing state between replicated
servers,
providing isolation and resource arbitration between multiple
tenants, and
globally optimizing network utilization.
\eat{Most of this complexity comes from
...
The process of troubleshooting these SDN platforms is hindered by the large number of hardware failures,
...
Our goal is to take a first step towards this goal by making it easier for developers
and operators
to reason about what causes the network to enter invalid configurations
(\ie~situations where the routing entries in switches do not
comply with a desired policy, \eg~inter-tenant isolation).
When we observe such invalid configurations,
which are \primafacie~evidence for bugs in the control software, our goal is
...
Our technique, \simulator, achieves this goal by automatically pruning inputs from the
original run, replaying the remaining
inputs to the control software with simulated network devices, and
checking whether the network re-enters the invalid configuration.
\eat{The output of \simulator, minimized input traces, represents a
noteworthy improvement over the status quo;
...
is the first to programmatically isolate fault-inducing inputs to SDN control platforms.

\eat{
In the remainder of this paper we provide an overview of \simulator,
and illustrate how it minimized input traces in a case study of three open source control
