...
checking. For POX, we additionally fabricated a buf in failover
...
Our first example is Frenetic~\cite{frenetic}, a control plaform 
...
When running learning switch, the simplest Frenetic application, we enountered
...
is that correspendence checking is a powerful mechanism for
...
Our second example is POX, another modern control platform.
...
We also tested the extreme limits of the simulator's scalabilty, pushing up
the number of switches until something broke. We enountered what appears to be
a limitiation of the Linux TCP/IP stack: TCP connection attempts began failing
