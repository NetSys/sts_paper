...
need two additional pieces of the diagnostic puzzle. First, in large distributed
systems with communication delay and hardware failures, transient policy-violations
are common and unavoidable; troubleshooters therefore need a mechanism to differentiate
...
\etc{}) that triggered the problem.
...
system execution, we are able to programatically (i) track the lifetime of
...
\noindent{\bf Policy-Violation Lifetime Tracking.} The first step in
...
synthetically generated or gathered
either from a production trace of failure and topology change
...
\noindent{\bf Causal Inference.} \colin{Need to be clear that we haven't
...
what I mean by "message send event", etc.}
%\begin{figure}[t]
%    %\hspace{-10pt}
...
topological traversal of the causal relation DAG, pruning leaves that are not
...
execution. Note that each iteration of the loop can be performed in parallel by cloning 
the state of the simulator. Note also that Lamport's definition of the
...
{\bf Additional Use-Cases.} Besides lifetime tracking and causal analysis, our simulation infrastructure has a
