...
As with any highly complex codebase, software bugs are an unpleasant but everpresent fact-of-life.
In fact, based on anecdotal evidence from colleagues in the industry, it seems clear that
developers of SDN systems spend much of their time
...
The troubleshooting process, which starts by analyzing long logs gathered from either operational monitoring or QA testing (though in this paper we focus on the latter), is hindered by the large number of hardware failures,
...
that these policy violations are rare and are symptoms of an underlying software bug (such violations could also result from a hardware failure, or a policy misconfiguration, but here we focus on those cases where the violation is due to a bug in the SDN control plane software).
...
distributed asynchronous systems rather than a single program running on a single server.
