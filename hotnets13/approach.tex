...
reproducing the
invariant violation. The most natural approach would be to maintain relative
timings between inputs to the extent possible. For instance, if we
define $t_i$ as the timestamp of the $i^{th}$ input from the original run and $t'_i$ as the replay clock value when it injects that same input (which may or may not be the $i$'th input in the subsequence), then the following is a plausible way to assign timings:
\begin{align*}
t'_0 = t_0 \\
t'_i = t'_{i-1} + |t_{i} - t_{i-1}|
\end{align*}
Unfortunately, this approach does not work in practice because, while it maintains relative timings, it can violate causal relationships.
...
The first issue is that internal events may differ syntactically (\eg~sequence numbers
of control packets may all differ) after modifying the original trace.
...
We would like to keep open the possibility of delta-debugging finding minimal causal sequences that do not contain those
...
In most cases this heuristic successfully reproduces the original invariant
...
thereby violating causality. If the event scheduling algorithm detects that it has waited too long, it
