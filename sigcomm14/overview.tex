...
Troubleshooting bugs found through QA testing is often an arduous task, even
though the majority of bugs developers deal with in practice are relatively
trivial. Fuzz tests frequently run for many hours before bugs are found, and involve
a large number of distributed nodes. Moreover, QA testing frameworks do little to mitigate
non-determinism (and often introduce non-determinism of their own that's
different from what would be seen in production or on developer's local
