...
QuickCheck's ``shrinking''~\cite{claessen2000quickcheck}) to bugs
in blackbox distributed systems.
In the domain of parallel computing these input minimization algorithms have
been applied to minimize thread
interleavings (but not inputs) leading up to race
conditions~\cite{choi2002isolating,claessen2009finding,jalbert2010trace,huang2012lean}.
...
telecommunications protocols~\cite{arts2006testing}, where it fed
commands to a switch and waited for a response. For our test cases we
tried replaying only wall-clock timings of external events but found that this
did not dependably reproduce invariant violations.
%without accounting for internal event interleavings.
