...
execution. Delta debugging has been applied to identify and prune threads from
execution traces that are not necessary for triggering concurrency bugs~\cite{huang2012lean},
and to minimize thread context switches leading up to race conditions~\cite{choi2002isolating}, but this still
...
may be inferred by replaying with code tracing enabled~\cite{tallam2007enabling})
in order to reduce the
length of deterministic replay executions. This work applies only to a single
machine, requires extensive code
instrumentation (including human annotation), and can be too conservative when
pruning.
