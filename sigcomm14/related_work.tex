...
In order to achieve this goal we are as far we know the first to extend
 extend delta debugging~\cite{Zeller:1999:YMP:318773.318946,Zeller:2002:SIF:506201.506206}
 to a distributed system, where there are multiple actors rather than a single point of
 execution and an ongoing sequence of inputs rather than a single flat file.
...
take through OpenFlow networks. Our goal is to transform such diagnostic
information to the minimum needed to understand what triggered a given bug.
...
which we believe fits more naturally into the way today's software engineers
have chosen to develop and test distributed systems.
...
the execution in question so that developers find it easier to find the
problematic code via replay or other means.
