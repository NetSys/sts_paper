...
in blackbox distributed systems.
In another domain, parallel computing, these input minimization algorithms have
been applied to minimize thread context switches
...
approaches still involve injecting a single input (the thread schedule) to a
...
tried replaying only wall-clock timings of external events, but we were not able to
dependably reproduce the violations without
