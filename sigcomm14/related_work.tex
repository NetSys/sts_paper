Our primary contribution, a technique for interleaving modified events,
...
QuickCheck's ``shrinking'' algorithm~\cite{claessen2000quickcheck}) to bugs
found in blackbox distributed systems.
In the context of concurrency bugs, input minimization algorithms have been applied
to minimize thread context switches
leading up to race conditions~\cite{choi2002isolating,claessen2009finding} and to
prune irrelevant threads from execution traces~\cite{huang2012lean}. These
applications still involve injecting a single input (the thread schedule) to a
single entity (a deterministic scheduler), and assume a high degree of
control over the system execution. QuickCheck has also
been applied in a blackbox fashion to test the synchronous part of
telecommunications protocols~\cite{arts2006testing}, where it fed sequences of
commands to a switch and waited for a response. For our test cases we
tried replaying only wall-clock timings of external events and waiting for the
violation to appear, but we were not able to reproduce the violations without
interleaving our external events with the (sometimes asynchronous) internal
...
or dynamically inferred~\cite{huang2012lean, tallam2007enabling})
...
required for instrumentation and annotation. We avoid making assumptions about
the language or instrumentation of the controller software.
