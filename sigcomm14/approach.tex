...
causal dependencies from the original log. The crucial point here is that when we replay only a
...
we need to
inject an input event $e$ only after all other
events (including internal events generated by the controllers, such as state changes or message
sends and receives) that precede it in the
