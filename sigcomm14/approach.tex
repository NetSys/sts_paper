...
(i) message deliver events, either between controllers or
between controllers and switches, and (ii) state transitions
within controllers (\eg~a backup node deciding to become master).
We obtain visibility into (i) by having our mock network
(to be described in \S\ref{sec:systems_challenges}) interpose on all message channels.
We optionally obtain visibility into (ii) by instrumenting controller
software (to be described in \S\ref{subsec:mitigating}).
With visibility into internal events and control over input events, the mock
network records a precise happens-before relation by logging an event only after
