Given a log $(E_L, T_L)$ generated from testing infrastructure,\footref{fn:log_gen}
our goal is to identify its MCS, so that a human can examine the MCS
...
Simply exploring subsequences $E_S$ of $E_L$ is insufficient for finding MCSes: the
...
wall-clock timings between the remaining inputs of each subsequence.
\eat{\ie~if we
...
We tried this simple approach on several buggy executions, but were rarely
able to reproduce their invariant violations. The reason is that the control software may react
differently to
remaining inputs as a result of pruning inputs, such that the remaining inputs
are injected at improper times. For example, a backup controller may elevate itself
to master at a different time during replay than it did with the original unmodified
inputs.
...
During replay our test orchestrator may use its interposition on
messages to reorder or drop messages as necessary to
remain consistent with the original happens-before relation. With this
interposition in place, the final MCS produced by our system includes a schedule
...
schedule supercedes the timings $T$ described in \S\ref{sec:formalism}.
...
observed at all in the original execution. We address each of these issues
while we replay each subsequence so that we can report invariant violations
