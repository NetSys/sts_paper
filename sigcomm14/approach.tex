...
searching for subsequences of $E_L$ \barath{Don't we want to say "searching for
subsequences of $E_L$ that still violate the invariant"?}, and searching for replay timings for those
subsequences.
...
the delta
debugging algorithm~\cite{Zeller:1999:YMP:318773.318946} provides a provably-correct
\barath{Do we want to say provably correct here given that the proof doesn't really apply
to us?} means of isolating fault-inducing inputs. In our case, we use delta
debugging to iteratively select subsequences of $E_L$ and replay each
subsequence with some timing $T$. If the bug persists, delta debugging ignores the
...
The delta debugging algorithm is shown in Figure~\ref{fig:ddmin} (with `$test$' replaced by `$replay$').
...
causal dependencies from the original execution: when we replay only a
...
event $i$ from the original log iff \barath{Is this "if and only if"?  If so, maybe expand
it.} all unmasked fields have the same value
...
\begin{figure}
  \begin{pseudocode}[framebox]{Peek}{events}
    ...
    \ENDPROCEDURE
  \end{pseudocode}
  \caption{{\tt Peek} is responsible for determining which internal events
  from the original sequence are going to occur for a given subsequence.
  \label{fig:peek}}
\end{figure}

{\bf Handling New Internal Events. } The last possible change induced by input pruning is the occurrence of new
...
has $O(n)$ complexity. \barath{Isn't this abusing big-O notation?  Is the intent here to
say that delta debugging is $\Omega(\log n)$ and $O(n)$?}

Each $replay$ takes $O(n)$ time
(one iteration for Peek(), and one iteration for the replay itself),
for an overall runtime of $O(n \log n)$ replayed inputs. \barath{The preceding sentence
...
allows us to replay starting from a recent snapshot rather than the beginning of the
