...
from~\cite{Zeller:1999:YMP:318773.318946}. $\subseteq$ and $\subset$ denote
...
decisions for each controller. This information is the starting point for
...
individual thread executions. % (\ie~causing them to exit early).
...
written in three different languages.\\[0.5ex]
%, for example those that depend on databases.}
%
...
%
\noindent {\bf Accounting for Interleavings.} To reproduce the invariant
violation (whenever $E_S$ is a supersequence of an MCS)
...
interposition layer (\S\ref{sec:systems_challenges}), we invoke {\sc Peek()} to infer absent internal events,
