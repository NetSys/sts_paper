...
operating conditions at first glance, making it difficult
to reason about the underlying root cause.
Consider that an hour long QA fuzz test, assuming 8.5 network error events per
...
We handle this possibility by attempting to infer the presence of internal
events before we replay each subsequence. Our algorithm for inferring the
present of internal events is depicted in
Figure~\ref{fig:peek}. The core technique is to inject each input, take a
snapshot\footnote{We discuss the implementation details of snapshotting in~\ref{subsec:snapshotting}} of
the network and the control software's state, allow the system to proceed up
until the following input (plus a small time $\epsilon$), record the observed events, and finally match up the
...
(functionally equivalent) causal predecessors have occurred.\footnote{In the
case that, due
to non-determinism, an internal event occurs during Peek() but does not occur
during replay, we time out for internal events after $\epsilon$ seconds of
...
Replaying a single subsequence chosen by delta debugging takes $O(N)$ time
(one iteration for Peek(), and one iteration for the replay itself.),
for an overall runtime of $O(nlog\,n)$ replayed inputs in the best case.
...
Storing periodic snapshots throughout testing can also reduce runtime, as we can
replay inputs starting from a recent snapshot rather than the beginning
of the trace.
