...
\colin{Needs to be updated, e.g. we now have distributed controllers.}
Our evaluation of \simulator~leaves open several questions, which we discuss
here.
...
not reproducible without adding significant complexity to the mock network. Our
...
assumptions about the controller platform in use. \num{Two of the three} investigated
...
to the size of the overall network, making it much easier to interpose on
messages.
...
In future work we hope to deploy and validate our
technique on other such systems such as NAS controllers or MapReduce
coordinators.

\noindent{\bf Dataflow analysis.} Another avenue for future work is combining \projectname's replay-based
minimization approach with dataflow analysis~\cite{kildall1973unified} to
further prune or preprocess events.
The simplest form of dataflow analysis involves
reasoning about happens-before relations (aka `potential causality'): if you consider a
software-defined network as a distributed state machine,
with individual processes sending messages between themselves, then one can safely prune an input event if
it does not induce any messages before
the occurrence of the invalid
configuration, since it could not have
possibly affected the outcome~\cite{Lamport:1978:TCO:359545.359563}.
Unfortunately we found that pruning inputs in this way does not
significantly reduce the number of inputs, but one might go further by analyzing the controller code that generates messages
...
split by time, but it may be more effective to try other approaches.
In particular, splitting events according to their location in the network
