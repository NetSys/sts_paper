...
& Pyretic loop & 3 switch mesh & 36 & 2 & Yes \\
% 20/20
% experiments/new_pyretic_loop_mcs/
% Total original inputs: 36
& POX premature packetin & 4 switch mesh & 102 & 2 & Yes \\
% 20/20
% Andi: experiments/pox_early_packetin_mcs
% Total inputs: 102
& POX in-flight blackhole & 2 switch mesh & 46 & 7 & Yes \\
% \colin{15/20 [20/20*]}
% Total inputs: 46
...
& POX migration blackhole & 4 switch mesh & 29 & 3 & Yes \\
% 20/20
% Total inputs: 29
...
& NOX discovery loop & 4 switch mesh & 150 & 18 & No \\
% 18/20
% Total inputs: 150
...
& Floodlight loop & 3 switch mesh & 117 & 13 & Yes \\
% 15/50
% Total inputs: 284
...
& Algorithm misimplementation & 3 switch mesh & 40 & 7 & No \\
...
& Multithreaded race condition & 10 switch mesh & 1596 & 2\footnote{The race
...
& Memory Leak & 2 switch mesh & 719 & 30 (M) & No \\
...
We now investigate the utility of \projectname~using multiple case studies. The
ultimate goal of \projectname~is to reduce developer effort for trouble-shooting
and debugging. As this metric is difficult to quantify directly, we
quantitatively compare the minimized and non-minimized event log sizes as a
proxy, and qualitatively relay our experience in handling the found bug. First,
we demonstrate \projectname's viability in troubleshooting real bugs. We found
\num{seven} new bugs by fuzz testing five open source SDN control platforms:
...
This case study demonstrates how even the simple handshake state machine can
...
\begin{subfigure}{0.32\textwidth}
    ...
    \caption[]{\label{fig:pyretic_loop} Pyretic loop. (total
    runtime: 266.2, original runtime: 5.8s)}
\end{subfigure}\hfill
\begin{subfigure}{0.32\textwidth}
    %\hspace{-10pt}
    \includegraphics[width=\textwidth]{../graphs/runtime/pox_early_packetin.pdf}
    \caption[]{\label{fig:pox_handshake} POX Premature
...
\begin{subfigure}{0.32\textwidth}
    ...
    \caption[]{\label{fig:pox_discovery} POX in-flight
...
\begin{subfigure}{0.32\textwidth}
    ...
    \caption[]{\label{fig:pox_migration} POX migration
...
\begin{subfigure}{0.32\textwidth}
    ...
    \caption[]{\label{fig:fl_loop} Floodlight loop. (total
...
\begin{figure}[t]
    %\hspace{-10pt}
\end{figure}
\noindent{\bf POX In-flight Blackhole.} Another bug was discovered using Fuzzing
...
There were $46$ inputs in the initial trace. While the trace provide difficult
to replay and minimize due to non-determinism in the system, we were able to
eliminate it by employing multiplexed sockets, overriding {\tt gettimeofday()},
...
----------------------------------- \noindent{\bf ONOS distributed database
locking.} When testing ONOS, a distributed open-source controller, we noticed
that ONOS controllers would occasionally reject switch attempts to connect upon
initialization. We minimized the (already small) sequence of startup events to 3
inputs. When examining the ONOS logs, we found that the particular timing
between the 3 input events caused both ONOS controllers to encounter a ``failed
to obtain lock'' error from their distributed graph database (Titan). We suspect
that the ONOS controllers were attempting to concurrently insert the same key,
which causes a known error in Titan~\num{cite}. We modified ONOS's
initialization logic to retry when inserting switches, and found that this
eliminated the bug.\\[0.5ex] \noindent{\bf Floodlight discovery loop.} Next we
...
both known and those synthetically injected by us.
...
input trace. As described in the case studies, we were able to successfully
counter the inherent non-determinism by applying the strategies described in
Sections \ref{subsec:mitigating} (for POX) and, as a fall-back \ref{subsec:coping}.
...
keep in mind ~$7 +/- 2$ aspects\cite{TODO_CITE_STUDY}}. Human debugging on large
input traces using standard debugging tools (log files, source level debuggers)
is tedious at best. Often, we tend to start working backwards from the crash. If
the actual root cause is many events in the past (e.g., in the Memory Corruption
example), this may require many re-iterations through the code and logs and
becomes intractable on human timescales. In contrast, it very easy to step
through a 2 to 7 event trace and manually identify the code paths responsible
for a failure.
...
\eat{
The most elusive class of bugs is those that are not always reproducible. These bugs are
...
approach on the minimization of a non-deterministic loop created by Floodlight in a 95-hour long
experiment. Figure~\ref{fig:non-determinism} demonstrates that the size of the resulting MCS decreases
...
\eat{
This is because the algorithm narrows its search every time
