...
& Reactive routing misimplementation & 3 switch mesh & 525.2 & 40 & 7 & Indirectly \\
...
%
\noindent{\bf Reactive routing misimplementation.}
...
One lesson worth noting is that in cases where non-determinism is particularly
acute, as in the reactive routing
misimplementation case study, \projectname's interposition on internal
events can actually negatively effect minimization due to time outs on
inferred internal events that do not occur during the replay run. In those cases we found
better results by simply turning off interposition on internal events.
Nonetheless, as described in most of the case studies, we were able to
counter the relevant sources of non-determinism
by replaying multiple times per subsequence and adding instrumentation. In
most cases we found better results by interposing on internal events.
