...
% - forwarding
% - spanning tree
% Just forget to specify priorities correctly.
% This "class" of bug is highly reactive, i.e. dependent on dataplane flows.
%
% Create a simple app that listens to PacketIns. Installs drop rule if
% certain MAC.
% Problem: HSA doesn't deal with priorities correctly ATM.
Decision based on last byte of TCP SYN & TBD & TBD & TBD \\
Timer misfire & TBD & TBD & TBD \\
% Already have an experiment, documented in gdoc
Algorithm misimplentation & TBD & TBD & TBD \\
% topo_proactive notes:
%  - It works on mininet with just ARP
%  - Console output is at /Users/cs/Research/UCB/without_dhcp.txt
...
% Could also use topo_proactive.
Multithreaded race condition & TBD & TBD & TBD \\
% Spawn a bunch of threads, and set up a quasi deterministic order that causes
% a crash. E.g. if 7th thread gets 15th PacketIn, crash.
Memory corruption & TBD & TBD & TBD \\
% -= 1 to a counter that should never be non-negative. But it is later, then
% crash.
...
% ONOS. Cause it to crash every once in awhile.
Latency spike & TBD & TBD & TBD \\
Invalid sleep value & TBD & TBD & TBD \\
% Latency spike could actually be folded into this -> longer sleep than
% expected -> queuing -> failover timers go off incorrectly.
...
The gold standard for evaluating \projectname~would be a large scale A/B test~\cite{neyman}
...
exception. We invoked \projectname~to identify a two element MCS:
a failure of a connected switch followed by a reboot/initialization of the same switch.
The nearly logarithmic runtime
behavior of \projectname~for this case is shown in Figure~\ref{fig:list_runtime}.
...
We measured the runtime of \projectname~for these case studies in Figures $6\;\&\;8-11$.
