...
& ONOS Database Locking & 2 switch mesh & N/A & 1 & 1 & - & N/A \\
% onos_db_lock_mcs/
\hline
\hline
...
& ONOS Master Election & 2 switch mesh & - & 20 & 4 & 4 & Yes \\
...
Our ultimate goal is to reduce effort spent on troubleshooting bugs.
...
%illustrate in detail how \projectname~found MCSes in the rest of this section.
Interactive visualizations and replayable event traces
...
\noindent{\bf Pyretic Loop.} We discovered a loop when fuzzing Pyretic's \verb=hub=
...
created, since the flow entries were still in place. The loop seemed to persist until
...
\noindent{\bf POX Premature PacketIn.} During a POX fuzz run, the
\verb=l2_multi= routing module failed unexpectedly with a \verb=KeyError=. The initial trace
...
instrumentation. The root cause was a race
condition in POX's handshake state machine. The OpenFlow standard
requires a 2-message handshake. % Afterwards, the switch is free to send arbitrary messages.
POX, however, requires an additional series of message
exchanges before notifying 
applications of its presence via a \emph{SwitchUp} event.

The switch was slow in completing the second part
...
\begin{subfigure}{0.33\textwidth}
    ...
    \caption[]{\label{fig:pox_handshake} POX Premature PacketIn.}
\end{subfigure}\hfill
\begin{subfigure}{0.33\textwidth}
...
\end{subfigure}
\caption{Minimization runtime behavior.}
\vskip -1em
\end{figure*}

\noindent{\bf POX In-Flight Blackhole.} We found a persistent
blackhole while POX was bootstrapping its discovery of link and host locations.
There were initially 27 inputs. The initial trace was affected by
...
and waiting on logging messages. \projectname~returned a 11 input MCS
...
We added host migrations to the randomly generated inputs and checked for blackholes. Our initial input size was
115 inputs.
...
\projectname~returned an 18 input MCS (Figure~\ref{fig:nox_discovery}).
...
packet loss. The loop was then installed when the source injected another
packet.\\[0.5ex]
\eat{ % -----------------------------------
...
instrumentation and increasing the log level each run. After 15 replay
...
the MCS, we were able to diagnose the root cause---a
...
\noindent{\bf ONOS Database Locking.} When testing ONOS, a distributed open-source controller, we noticed
...
%
\noindent{\bf Floodlight Failover.} We were able to reproduce a
known problem~\cite{floodlight_bug} in Floodlight's distributed controller failover logic with
...
\noindent{\bf ONOS Master Election.} We reproduced another bug,
...
initial IDs chosen at random, so we modified ONOS to hardcode ID values.
With interposition we observed timeouts on LLDP messages, which are sent at
random. Interposition was not necessary for this bug however,
and we found an MCS more quickly without it.\\[0.5ex]
%
\noindent{\bf POX Load Balancer.} We are aware that POX
...
reproduce the bug during replay after five attempts. %, and were left with the original 39 input trace. 
This is the only case where we were
unable to replay the trace.\\[0.5ex]
%
\noindent{\bf Reactive Routing Trigger.}
We modified POX's reactive routing module to create a loop upon receiving a
particular sequence of dataplane packets. This case is difficult for two
reasons: the routing module's behavior depends on the (non-deterministic)
order these links are discovered in the network, and the triggering
events are multiple dataplane packet arrivals interleaved at a fine
granularity.
We found that the 7 event MCS was inflated by
at least two events: a link failure
and a link recovery that we did not believe were relevant to triggering the bug.
We noticed that after {\sc Peek()} inferred expected internal events, our
event scheduler still timed out on some link discovery messages--those that
happened to occur during the {\sc Peek()} run but did not show up during
replay due to non-determinism. We suspected that these timeouts
were the cause of the inflated MCS, and
confirmed our intuition by turning off interposition on internal events
altogether, which yielded a 2 event MCS (although this MCS was still affected by
non-determinism).\\[0.5ex]
%
...
expected triggering conditions: control channel congestion followed
...
subsequence, we were able to minimize a 1596 input in 10 hours to a
replayable 2 element failure/recovery pair.
...
We created a case where the receipt of a
...
to mitigate the effects of non-determinism. We evaluate the effectiveness of this
approach by varying the maximum number of replays per subsequence while
minimizing a synthetic non-deterministic loop created by
Floodlight. Figure~\ref{fig:non-determinism} demonstrates that the size of the resulting MCS decreases
with the maximum number of replays, at the cost of additional runtime;
10 replays per subsequence took 12.8 total hours, versus 6.1 hours without
retries.
...
For POX and Floodlight, we added shim layers to the control software
to redirect {\tt gettimeofday()}, interpose on logging
statements, and demultiplex sockets. For Floodlight we needed 722 lines of
Java, and for POX we needed 415 lines of Python.
...
Note that \projectname~is not designed for high-throughput dataplane
...
\begin{figure}[t]
    ...
    \caption[]{\label{fig:scaling} Runtime for bootstrapping FatTree
...
packet drop rate dropped too many LLDP packets, preventing the controller from discovering the topology.
...
how long an invariant violation should persist before
...
We found that the number of events we timed out on while isolating the MCS became stable for values above 25 milliseconds.
