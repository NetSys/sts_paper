...
Multithreaded race condition & 10 switch mesh & TBD & 2\footnote{The race
...
troubleshooting real bugs. We found \num{five} new bugs by fuzz testing open source
...
\noindent{\bf Floodlight discovery loop.} We
...
\noindent{\bf Floodlight example failover bug.}  We were able to reproduce the problem shown in Figure~\ref{fig:example} with
\projectname. We ran two Floodlight controller
instances (a version before the bug was fixed), connected to two switches, and injected 200 extraneous link
...
We were able to successfully isolate the two event MCS.
...
Here we show how much effort it took us to interpose on logging, gettimeofday.
