...
\noindent{\bf POX Premature Packet-In Failure.} We discovered this bug accidentally
while working on a synthetic test case. During a particular fuzzing run, the
\emph{l2\_multi} module failed unexpectedly with a KeyError. The initial trace
...
instrumentation to the POX code. The root cause turned out to be a race
condition in POX's handshake state machine. Note that the OpenFlow standard
...
exchanges before considering the switch fully connected and notify the
application modules of its presence via a \emph{SwitchUp} event. Until then, it
already forwards events triggered by switch messages. 
...
\emph{l2\_multi}. \emph{l2\_multi} would then fail because it relied on the
invariant that \emph{SwitchUp} would occur first. This case study demonstrates
how even the simple handshake state machine can behave unexpectedly in a
non-trivial way that is hard to understand without being able to repeat the
...
\begin{figure}[t]
    ...
    \caption[]{\label{fig:pox_handshake} Minimizing the POX Premature Packet-In Failure}
\end{figure}
...
In addition to our troubleshooting case studies, we strive to show that
\projectname~is able to successfully minimize traces for a range of bugs. We
show this on a range of bug types, both known and synthetically injected by
us.
...
\tbd{Explain in rough terms what the categories are, and explain for the cases that
didn't work so well why we didn't expect them to work so well.}
