...
The gold standard for evaluating \projectname~would be a large scale A/B test
...
expert developers are scarce; to achieve statistical significance we would
need 10s to 100s of developers to spend 5 or more hours on each bug we find.
In addition we lack access to the QA testbeds of the scale and complexity
used to test production controller software.

We instead begin our evaluation by applying \projectname~to find bugs in three open source and one proprietary SDN control platform:
Floodlight~\cite{bigswitch}, ONOS~\cite{ONOS}, POX~\cite{pox}, and NOX~\cite{nox}.
The remainder of our evaluation
involves qualitatively approximating A/B testing with in-depth case
studies on the troubleshooting process with and without minimized traces.
...
user to set the rates at which different event types are triggered. In our initial experiments with
...
In one case we discovered that a high dataplane
packet drop rate dropped too many LLDP packets, preventing the controller from ever successfully discovering the topology.
In another case we found that setting
controller failure rates too high would sometimes cause the entire distributed
controller cluster to
go down or become partitioned, which creates situations that developers assume
will not occur in practice. Tweaking fuzzing parameters remains an important
...
developers. We therefore need to set manually set a threshold
...
\noindent{\bf Setting Epsilon.} Our algorithm leaves an open question as to what value
