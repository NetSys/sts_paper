...
given a faulty execution trace, attempts to automatically eliminate
...
order to dependably reproduce bugs throughout the minimization process.
We aim to account for these interleavings without making assumptions about the language
or instrumentation of the control software.
As far as we know we are the first to fulfill these requirements
...
commercial SDN company to integrate \projectname. We evaluate the effectiveness of {\projectname} in
...
failure-induced blackholes, internal-state errors,
and distributed lock acquisition---that we found
...
minimize the newly discovered bug traces by up to 98\%, and we anecdotally found that reducing
bug traces to MCSes made it easy to understand their root causes.
