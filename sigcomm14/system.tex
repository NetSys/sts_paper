...
controller by telling it to {\texttt fork()} itself and suspend its child,
and later resume the child after cloning the original sockets of the parent
(which constitute shared state between the parent and the child processes,
since the socket state is managed by the kernel). This simple mechanism does
not work for controllers that use other shared state, such as disk.
To handle other shared state one could either use Unix containers or virtual
machine snapshotting. For distributed controllers, one
...
If users of \projectname~do not choose to implement a snapshotting mechanism,
...
simply employ some of the event scheduling heuristics described in
