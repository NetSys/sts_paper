...
pinpoint the root causes of bugs. We therefore implemented a number of root
causing tools within \projectname,
which we use along with Unix utilities such as `grep' to help us complete the final
stage of debugging.
...
Because the timing of events in  distributed systems is so delicate,
this mode of \projectname~does not run concurrent controller processes,
but instead mocks out
...
\colin{Note that there are NO widely used tools that help with this kind of
non-determinism! For the most part research projects designed to aid with
this problem have not left the ivory tower. We are no worse than the status quo!}
