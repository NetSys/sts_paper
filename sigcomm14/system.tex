...
top of this \tester~and connect the emulated software switches to the
controllers. % as if they were true network devices
%, such that the controllers believe they are configuring a true 
%network.
Beyond implementing delta debugging, the \tester~interposes and buffers messages on all communication
...
underlying probability $p$, then $r$
replays will observe the bug with probability $1-(1-p)^{r}$. This exponential
...
We therefore struck a balance between resilience to non-determinism and the need
