...
the order in which messages are let through. The overall simulation architecture is depicted in
...
liveness, (e) POX ACL compliance, and (f) ONOS flow routing compliance.
...
state transitions does not handle all cases, but we find it suffices in practice.\footnote{We discuss this limitation further in \S\ref{subsec:non_goals}.} Beyond
...
Beyond its use in {\sc Peek()}, snapshotting has three advantages as a primitive
...
\S\ref{subsec:algorithm}) ensure that invariant violations are reliably
...
even minimized event traces were insufficient to
...
output a non-minimal causal sequence. In the common case this is still much
