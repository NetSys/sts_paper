...
Before describing the many ways in which such a system might fail, we note
that there are two different kinds of SDN control applications: proactive and
reactive. \colin{Reviewer OE: this claim seems to leave out the vast middle
ground}
...
\colin{Urs: I think one of the things that is going to cause some agnst, and
justifiably, in the industry, is that if you have a controller, you don't want
your controller to be a SPOF, so you have a backup controller, you need to
have some kinds of master election. That stuff is hard to get right}
...
production networks \colin{Reviewer OE: this is false. There is a level of
risk beyond which sysadmins just do not care about
rare events. Before we introduce so much new mechanism for
