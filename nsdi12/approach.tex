...
\colin{Reviewer B: Without an introduction and an explanation of how the
...
control plane based on the input trace\colin{Use the word ``Forensically''.
Need to be more concrete about how this tool actually gets used}. Throughout the system execution,
the simulator periodically\colin{Reviewer OB: Why not invoke it every time
there's a new event being evaluated? If the periodic checking is justified,
what is an appropriate period?} invokes correspondence checking to enumerate all
...
Without regarding causal constraints, a naive algorithm to infer the minimal
...
\colin{
Panda and I realized that the minimal causal set algorithm we presented in the OSDI submission is erroneous.
...
\item Need to make a distinction between "random events" (e.g. link failures,
node failures) and "deterministic events" (e.g., a node is programmed to send
a pong when it receives a ping). In the MCS algorithm, it makes no sense to
prune out deterministic events... However, the algorithm still needs to take
in the entire causal DAG (both random and deterministic) to maintain causal dependencies [can't prune arbitrary random events]
...
causally-related. In fact, as described here, our algorithm can be viewed as a
