Including vector clocks on network control messages
enables automated diagnosis of control plane bugs. In this section we 
describe our approach for finding the minimal causal
set of events that triggered a given failure. As a pre-requisite to causal
inference, we present correspondence checking, which allows us to identify
policy-violations without needing to explicitly define invariants. Our causal
inference then repeatedly invokes correspondence checking in a deterministic
replay of the system execution, pruning events not related to the given
policy-violation.
...
believe.} \colin{Also note that in general, specifying invariants is an
extremeley hard problem. Yes, it is a weak condition, but that's sort of the
point -- it doesn't require manual intervention. Also, if you wanted a more
specific blackbox detector, you could write one.}
...
\colin{reviewer B: The problem with this definition is that it allows
implementations of virtual networks that we would intuitively say are wrong.
In particular, forwarding at the physical level could be implemented in a way
that is not compatible with the virtual-to-physical mapping. For example, the
physical configuration might send all traffic over a single link, even though
the virtual configuration uses multiple links and the virtual-to-physical
mapping is bijective. Or instead of actually dropping packets, the physical
configuration could send them into a forwarding loop. As long as
the packets ultimately arrive (or do not arrive) at their ultimate
destination, your definition allows it. To get an equivalence that does not
suffer from this problem, you might consult the literature on bisimulation
equivalence (Robin Milner's small book on CCS and the Pi-calculus is an
excellent starting point; Xavier Leroy's work on the CompCert verified
compiler is a nice example of this in action). A bisimulation would require
that every action at the virtual level can be simulated by an equivalent
action at the physical level, and vice versa.}
...
come from IS-IS. Controller crashes and switch failrues come from heartbeats. 
...
\item Vector clocks as an optimizatoin for causal inference. (If concurrent,
