...
Next, our algorithm requires a deterministic execution environment, capable
replaying the system execution in a controlled fashion. We have developed
solutions to two separate challenges in achieving this requirement.

First, replaying events deterministically. Run all controllers in VMs, and
proceed forward in lock step. Then, map logs events onto actions reproduced in
the simulator. For example, simulate the network entities, and simulate link
failures by disconnecting the edge in simulated link. Or emulate node crashes kill -9.
Replay policy-changes by plugging into Quantum. Note that we don't need
a precise reproduction of the failure on an /individual/ node.    * Distributed systems are built under the assumption that individual
   ...
   * We aren't really interested in diagnosing individual component
     ...
     our oscilloscopes and find the exact circuit path that's going wonky)
