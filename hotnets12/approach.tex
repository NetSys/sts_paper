...
relation~\cite{Lamport:1978:TCO:359545.359563}. The only remaining issue is
that some failure events may appear multiple times in the log (\eg{} multiple
replica servers detecting that a master went down), yet we only want to
replay the original event. If it is not possible to distinguish the original
failure event from the consequential events (\eg{} in the
...
OpenFlow $PORT_STATUS$ message from the switch to its parent controller. The simulator replays policy-changes by
...
relation ($P = {i \mid i \rightarrow e}$) have occured. Unfortunately the problem is made
...
syntactic differences (\eq{} sequence numbers on packets) by
uniquely identifying events according to their place in the causal tree.

\colin{
Note that in the absense of causal annotations it is still feasible to replay
the system execution by naively injecting inputs using timers, although the
output of the algorithm is not guarenteed to be exactly correct.
}
