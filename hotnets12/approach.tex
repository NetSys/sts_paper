...
Happens-before is to be distinguished by causality. appens-before says "could
have caused". Casuality says "actually affected the behavior"
...
Solution #1: The simulated system includes causal annotations (re: X-Trace) on all messages and internal events. 

Solution #2: Andi had an idea for setting a "time window" -- basically, every time we prune, run the execution twice. On the first run, only inject one external input, record all internal events, and let the system run until it stabilizes. On the second run inject the next external input right at the point when you observed the system stabilize. We're not quite sure this works though -- it also seems like an O(n^3) algorithm to me.
