...
and hosts in the network. We also run the controllers in individual virtual
machines that are
...
causal offspring for any given event. Since we are not interested in casual annotations
for many layers as in X-Trace~\cite{xtrace}, adding these annotations is a relatively
straightforward process. When we prune an input $e$, we then remove
...
appropriately. We also use the special value $LOOP$ to distinguish
