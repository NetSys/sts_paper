...
\begin{algorithmic}
\State $MCS \gets []$
\ForAll {\text{\it external input in trace}}
    \State {\it remove first input from trace}
    \State {\it bootstrap system}
    \State {\it simulate system execution until a fixed point}
    \State $snapshot \gets snapshot\_system()$
    \State $violation \gets detect\_violation(snapshot)$
    \If {\it not violation}
        \State $MCS << event$
        \State {reinsert input to front of trace}
    \EndIf
\EndFor
\end{algorithmic}
...
% ------ Maybe inlude this quote from Google -------
