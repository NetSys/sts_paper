...
\ForAll {\text{\it external input in log}}
    ...
    \If {\it not violation}
        \State {\it add event to MCS}
        \State {\it reinsert input to front of log}
    \EndIf
\EndFor
\end{algorithmic}
...
checking algorithm for detecing errant behavior.
...
network devices, and the controllers believe they are configuring a true
network. The simulator interposes on all communication channels in
the network, allowing it to proceed with the system execution in
...
It returns a list of inconsistencies between the virtual view and the
simulated network, and the troubleshooter specifies the signature of the
policy-violation of interest so that its presense in the list can be checked.
