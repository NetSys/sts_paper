...
\colin{@Peter: isnt your idea to refine the defn of causality just X-Trace?}

\colin{What distributed systems primitives would we want out of network
       devices? Transactions for consistency, vector clocks for causual
       analysis + alternate consistency models, support for distributed locking?}

\colin{TODO: read the NVP whitepaper. Floodlights too, if its available}

\colin{Scott: I think W3 implicitly assumes statefullness in the controller.
       Does it even apply to NVP? I suppose there is state in the NVP controller
       still though. For example, each controller has a boolean flag: Am I
       Master}
...
%distrbuted system of its own}. We argue that switches and routers should be
