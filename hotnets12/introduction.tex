...
Modern software-defined networking control platforms are highly complex,
...
Constructing and operating tightly-coupled control clusters also presents a
set of challenges that the networking community has little experience
solving. For example, as Google's Urs H$\ddot{\mathrm{o}}$elzle noted at a recent
keynote~\cite{urs_keynote}: ``[controller coordination] is going to
cause some agnst, and justifiably, in the industry''.

As industry deploys SDN on larger and larger networks, the complexity of the control
software will only continue to increase, and with complexity inevitably always comes
bugs. To make matters worse, in large networks the rate of hardware failures,
policy-changes, and other inputs to the system can be so great that the task 
of finding the set of inputs that caused the system to enter an erroneous state
is highly tedious and time consuming.

In this paper we tackle the problem of automatically inferring the minimal set
of events that is sufficient for
triggering policy-violations discovered in executions of the system.
Our hope is that by automatically providing an exact diagnosis of the
triggering events, operators and developers can be free to focus their efforts
on debugging the code itself rather the reproducing the fault in the first
place.

Our algorithm, \simulator{}, leverages (i)
infrastructure for analyzing distributed logs (ii) deterministic replay
simulation environments, and (iii) invariant verification techniques.
In addition to detailing these components, we describe a preliminary case-study
on the Floodlight production controller~\cite{floodlight}, where we reproduce a
corner-case failover scenario between two controllers resulting in a permanent
network blackhole. 
...
In \S\ref{sec:casestudy} we describe 
the Floodlight case-study. Finally, in \S\ref{sec:related_work} we provide a
discussion along with related work,
