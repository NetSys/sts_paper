...
\Simulator{} is certainly not without limitations. Here we discuss these
limitations, and provide how the assumptions
detailed in Section~\ref{sec:approach} can be relaxed.

\noindent{\bf How is the global log obtained?} Our approach conceptually depends on a correctly ordered, global log of the
...
\noindent{\bf Are the component failures reproduced by the simulator really
indistinguishable from the failures that occured in the production run?} If the
...
causally independent?} This depends on what we are trying to debug. Consider a drunk network operator who (i) trips on an ethernet cable, and
...
(ii). Our goal is to model drunk operators though. Our goal is to
test the overall system's reactions to errant behavior on one or more
individual nodes. In the case of the drunk operator, what we {\em do} care
about is whether the system reacted incorrectly to the two events\footnote{We
leave it up to management to deal with the drunk operator's behavior}

\noindent{\bf What about random number generators or other non-deterministic
behavior?}

\noindent{\bf Will this approach work on bare-metal (non-Onix) controllers?}

\noindent{Once you have a system the distributes rules correctly and
consistently and this layer is correctly implemented... then do you
ever need W3 again? or is it the position that this won't happen and
so we'll always need to debug rule distribution?}

\noindent{\bf Isn't this work applicable to all distributed systems, not just
SDN?}
