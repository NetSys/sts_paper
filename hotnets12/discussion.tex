...
inputs to the system. If the log is too long to rerun from the beginning for
every iteration, the operator can take causally-consistent
snapshots~\cite{Chandy:1985:DSD:214451.214456} of the
live system and bootrap the simulator from the nearest quiescent snapshot.
...
(ii). The developer's goal is not to debug drunk operators though.
...
assumes that it has access to a physical view with a routing table-like format.
