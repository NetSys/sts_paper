...
our goal is to identify its MCS so that a human does not need to examine
the full log. This involves two tasks:
...
\end{center}
\end{boxedminipage}
\caption{\colin{Cut if we need space}\textbf{Automated Delta Debugging Algorithm
...
MCSes: the timing of when we inject the external events is crucial for
...
we say that the inputs are isolated.
The original applications of delta
...
when applied to blackbox systems like synchronous telecommunications protocols~\cite{arts2006testing}.
\eat{\ie~if we
...
can reorder or delay messages, or that
controllers often process multiple inputs simulataneously.
...
\noindent {\bf Allowing Divergence.} Rather than recording all
...
By virtue of managing inputs and message deliveries centrally,
we are able to totally-order the event trace $\tau_L$.
%We record the totally-ordered trace $\tau_L$ by logging an event only after
%all prior events are completed.
%Our test orchestrator may additionally use its interposition on
%messages to reorder or drop messages as necessary during replay to maintain the
%happens-before relation.

%Note that we do not control the occurrence of internal events, and therefore do
%not attempt to minimize them. To find minimization opportunties however we need
%to carefully interleave them with external events.
%Crucially though, we need to ensure that the ordering
...
throughout replay of subsequences of the
...
\hline
\end{tabular}
\caption{\textbf{Internal messages and their masked values.}%The masks serve to
...
\begin{figure}
  ...
  \begin{pseudocode}[framebox]{Peek}{events}
    ...
    \ENDPROCEDURE
  \end{pseudocode}
  \caption{\textbf{{\sc Peek} determines which internal events
  from the original sequence occur for a given subsequence.}
  \label{fig:peek}}
  \vskip -1.5em
\end{figure}

\noindent {\bf Handling New Internal Events.} The last possible induced change is the occurrence of
...
beginning.% \barath{But doesn't this approach break due to non-determinism?  Sometimes we want
