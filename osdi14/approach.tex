...
subsequences that are a superset of some MCS, we say that the system execution is serializable.
...
which seek to minimize thread interleavings leading up to race conditions.
...
By fixing the inputs, these approaches are forced to stay on the same code
path from the original run, and are unable to find alternate paths that still
...
We record the totally-ordered trace $\tau_L$ during testing by logging an event only after
