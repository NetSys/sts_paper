...
(iii) replay, and
(iv) root cause analysis (of device failures).
...
take through OpenFlow networks. These tools allow developers to understand
...
the system~\cite{pip}. Within networking, this research is
...
We produce exact minimal sequences, and we focus on correctness
...
allow users to interactively step through (deterministic) executions in exchange for runtime recording overhead.
...
devices have failed). Root cause analysis~\cite{yemini1996,Kandula:2009:DDE:1592568.1592597}
