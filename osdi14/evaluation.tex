...
\hline
\end{tabular}
\caption{\textbf{Overview of Case Studies.}}
...
%at \href{http://ucb-sts.github.com/experiments}{ucb-sts.github.com/experiments}.
...
\noindent{\bf NOX Discovery Loop.} We tested NOX on a four-node mesh, and discovered a
...
The destination host sent one packet, and NOX initially learned
...
The controller exhibited significant non-determinism. %, which initially
...
We repeatedly replayed the MCS while successively adding
log instrumentation to Floodlight. After 15 replay
...
host to flap between perceived attachment points.
...
%We evaluate
%\projectname's minimization of previously known bugs.\\[0.5ex]
%
...
%We lastly demonstrate the
%boundaries of where \projectname~does not work well by injecting and
%minimizing synthetic bugs into POX.\\[0.5ex]
%
...
%The final MCS also was not replayable on the first try.
...
expected conditions that triggered that code path: control channel congestion
followed by decongestion.\\[0.5ex]
%
...
%This suggests that replaying each subsequence multiple
...
\hline
65 & 20 & 15 & 12 & 9 & 9 & 9 & 6 & 6 & 6 \\
\hline
6 & 7 & 8 & 10 & 6 & 11 & 12 & 12 & 11 & 13 \\
\hline
%1 & 65 & 6.10 \\\hline
%2 & 20 & 6.37 \\\hline
%3 & 15 & 7.78 \\\hline
...
%10 & 6 & 12.86 \\\hline
\end{tabular}
\caption[]{\label{tab:non-determinism}
%\textbf{Effectiveness of replaying subsequences multiple times in mitigating non-determinism.}
\textbf{Max replays per subsequence vs. MCS size\newline vs. total runtime in
hours.}
}
\vskip -2em
\end{table}
...
%10 & 6 & 12.86 \\\hline
%\end{tabular}
%\caption[]{\label{tab:non-determinism}\textbf{Effectiveness of replaying
...
\begin{figure}[t]
    ...
    %Simulation time for bootstrapping FatTree
    %networks, cutting 5\% of links, and processing the controller's
    %response.}
\end{figure}

\eat{ % ADD BACK IN IF WE HAVE SPACE!
...
} % END EAT OF PARAMATERS SUBSECTION
