...
policy changes via a management interface, and possibly dataplane packets.
In response, the
...
external (input) events such as failures on large (software emulated or hardware) network testbeds.
If they detect an invariant violation, they hand the resulting trace to a developer for analysis.

The space of possible bugs is enormous, and it is time
