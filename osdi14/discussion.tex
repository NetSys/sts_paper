...
Moreover, control plane systems like SDN repeatedly return to
quiescent states, which means that many inputs can be pruned.
...
%technique on replicated state machines and distributed databases. % such as replicated state machines or NAS
%% controllers.
...
need to be logged in sufficient detail for \projectname~to
replay a synthetic version. Finally, without care, a single
event may appear multiple times in the
