...
shot? That is, was the event of his death intrinsic to the precipitation of the war, or
...
In computer systems however, it is possible to do better:
...
greatly from technology for programmatically answering such questions. A 2006 survey found
...
Troubleshooters find little immediate use from logs containing many events
prior to a fault,
since they are often forced to manually filter events that are
characteristic of normal operating
conditions before they can make headway in diagnosing the problem.
...
% Colin: we haven't said "in this paper, we do X" anywhere in the
% introduction. Maybe not a problem.
...
software under test.
Pruning inputs, however, may cause the remaining events to
subtly change (\eg~the sequence numbers of packets may all differ), and some
events that occurred in the original
run may not occur.
Our approach is to apply a small amount of domain knowledge codifying the
syntax and semantics of events. Masking over semantically insignificant
information allows us to reason about
differences between slightly altered histories and thereby reproduce the error
whenever possible throughout the pruning process.

\colin{Rather than `apply domain knowledge': `operating at the application
layer rather than I/O and interrupts is the only way to get access to the semantic information to be able to
reason about divergence. Make it so that we don't have to apologize for
heuristics.}
