...
solving in tightly coupled environments. \colin{Scott: Should we tone this
down?}
...
One straightforward approach to pruning extraneous inputs is to account for
potential causality: if an input does not induce any messages that arrive
before the occurence of the invalid configuration, it
cannot possibly have affected the outcome~\cite{Lamport:1978:TCO:359545.359563}.
Unfortunately, pruning concurrent inputs alone does not significantly
significantly reduce the number of inputs.

Our approach is to iteratively
prune inputs from the original run, replay altered histories, and
