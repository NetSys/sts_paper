...
Many historians ascribe the start of World War \rom{1} to the shooting of Archduke Ferdinand,
but others point out that the underlying causes of the war went beyond that single act.
Would WW\rom{1} still have occurred without the shooting? Unfortunately we can never know such
historical counterfactuals for sure. In computer systems however, it is possible to do better:
the ability to prune and replay system traces allows one to verify whether
certain unfavorable outcomes occur in alternate histories.

Software bug diagnosis, a highly expensive and time consuming process, would benefit
greatly from a tool for programmatically answering such questions. A 2006 survey found
...
since they are often forced to manually distinguish events that are
...
syntax and semantics of events. Masking over semantically insignificant
information allows us to reason about
differences between slightly altered histories and thereby reliably reproduce the
