...
triggered the bug, and conversely filter out those that are extraneous.

One straightforward approach is to account for
potential causality: if an input does not induce any messages before
...
In particular, we extend delta debugging~\cite{Zeller:2002:SIF:506201.506206}--an algorithm for
...
achieve the same end, but they depend on manual inspection and pruning
