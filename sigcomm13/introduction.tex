...
Unlike political history, it is possible to prune and replay traces of computer
system behavior, making it feasible to answer
questions of the form ``If this event had not been present, would the
system's configuration have differed significantly?''.

Tools for programmatically answering such questions are useful for bug diagnosis,
...
observed in production environments. As one data point, Microsoft Research
reports 8.5 network error events per minute per
datacenter~\cite{Greenberg:2009:VSF:1592568.1592576}.
Troubleshooters find little immediate use from large traces,
since they are forced to manually distinguish events that are
characteristic of normal operating
conditions from those that push the network into an erroneous state before
they can make headway in fixing the root cause.
...
%\andi{We should emphasize the divergence problem more. This is
%not some subtle extension of getting the timing right, but a fundamental limitation with changing history
%that has stopped people from sucessfully applying such approaches.}
...
reliably reproduce errors by recording the exact sequence of inputs to the
software under test.
Pruning inputs however, causes the remaining events to
subtly change (\eg~the sequence numbers of packets may all differ), and some
internal state changes that occurred in the original
run may not occur after pruning prior inputs.
Our approach is to apply
a small amount of domain knowledge codifying the
syntax and semantics of events,
allowing us to reason about the
differences between slightly altered histories and reliably reproduce the
error throughout the pruning process.

Related tools such as {\tt ndb}~\cite{handigol2012debugger} provide
a historical view into dataplane (mis)behavior. \Simulator~instead provides
...
can be used to gather similar information, but they depend on manual pruning
and analysis to do so. As far as we know, our work
