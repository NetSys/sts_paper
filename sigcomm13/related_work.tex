...
recent project NDB~\cite{handigol2012debugger}. OFRewind focusses on enabling
record and replay capabilities on a production OpenFlow domain, and
...
the search of minimal input traces.

NDB uses a similar set of technologies as OFRewind and provides event tracing
and 'breakpoint' like functionality for individual packets. Thus, NDB can
provide a microscopic trace view to the OpenFlow forwarding configuration
encountered by a particular packet in the network. We find that in our
experience, problems with OpenFlow forwarding tables are relatively simple to
debug. Challenging bugs often result from interference on the control plane
decisions.

Neither of the two projects focusses on the problem of information overflow
