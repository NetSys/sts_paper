...
We share the common goal of practically improving the troubleshooting
possibilities for Software Defined networks with OFRewind~\cite{ofrewind} and
recent project NDB~\cite{handigol2012debugger}. OFRewind focuses on enabling
record and replay capabilities of OpenFlow control channels, and
...
NDB uses a similar set of technologies as OFRewind,  providing event tracing
and `breakpoint'-like functionality for (historical) dataplane packets. Thus, NDB can
provide a trace view into the OpenFlow forwarding tables
encountered by particular packets in the network. We have found in our
experience that examining low-level forwarding behavior is not
particularly helpful for identifying what is often the root cause of the problem: the
control plane decisions that installed the erroneous routing entries in the first place.

Neither of the two projects focuses on the problem of information overflow
during debugging - with millions going over the packets on the wire, it can be
very challenging to pick just the right one to debug interactively with NDB, and
a plain replay does not necessarily facilitate root cause analysis. To the best
of our knowledge, \simulator~is the first system that automatically provides
