...
was POX, the successor of NOX. \colin{Cut this sentence:} POX
...
\projectname~made it straightforward to inject inputs (in this case, randomly generated)
...
We noticed that \projectname~reported a blackhole while POX was bootstrapping its
...
In the end, we found that the crucial triggering event was an {\em in-
flight} packet. The root cause in the code was a missing return statement in a
nested conditional.


While the discovery module sent out LLDP packets to learn links,
there were six traffic injection events between hosts ($A\rightarrow B$ and
$B\rightarrow A$). POX was learning the locations of the hosts in the following pattern: host $A$
...
$A$ at port $1.2$, would flood the packet to the unknown destination $B$, the
...
switches and flushed its incorrect learned location of $A$ when it discovered
...
in-flight} packet from $A\rightarrow B$ arrived at port $2.1$, and POX incorrectly
learned $A$'s location at $S2$ (this time without a prior notification from
$S1$ about the $A\rightarrow B$ flow).
POX now believed that $A$ and $B$ were attached to the
same switch, and proceeded to install a (correct) flow entry
in $S2$ from $2.1$ to $2.2$ for $A$'s flow to $B$.
...
POX discovery blackhole and the POX list removal bugs.
We found for both cases that the numbers of events we timed out on while isolating the MCS became stable for values above $25$ milliseconds.
