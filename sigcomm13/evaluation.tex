...
(which involved multiple OpenFlow initialization handshakes), this is not entirely
unsurprising.
...
the fuzzer found an event sequence with 777 total events
(548 input, 229 internal) that results in a 3-node forwarding loop being set up.

Floodlight makes use multiple kernel level
...
\Simulator~was able to reduce the number of input events from
548 to 404 in 168 iterations (note that this is not the final result; the
algorithm crashed due to a trivial error a few hours before the deadline).
...
which took more than 5 and a half hours. Nonetheless, we conjecture that even long iteration
...
On a machine with 6GB of memory, the runtime of creating a FatTree, performing
OpenFlow handshakes, cutting 5% of links, and processing the response scales
linearly up to 2464 switches [a 45-pod FatTree]. After that, the machine
started thrashing.

(Took me 3 days to get this result: had to track down a really nasty bug that
only showed up at scale.. On the plus side, I made initialization an order of
magnitude faster)
