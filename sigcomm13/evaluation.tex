...
Floodlight loop & 3 switch mesh & 15/50 & \num{?} & \num{?} \\
% Andi
\end{tabular}
...
is a single-machine control platform intended primarily for research prototyping and
...
\noindent{\bf POX in-flight blackhole.}
...
Before the discovery module had learned of the link,
there were six traffic injection events between hosts (A$\rightarrow$ B and
B$\rightarrow$A). At the point when the link was discovered, POX had learned
of B's location at port $2.2$. %Upon  POX would oscillate between learning the correct location
...
We next observed an {\em in-flight} packet arriving from A$\rightarrow$B at
port $2.1$
...
switch-switch link discovery. As a result,
POX incorrectly learned A's location at 2.1. If the first packet had
...
1.1$\rightarrow$2.1 and 2.2$\rightarrow$2.1, whereas it should have installed
...
packet, and
failed to return out of a nested conditional as result the second in-flight
...
migrations this time, and checked for blackholes. Our initial input size was 117 inputs.
...
\noindent{\bf Floodlight discovery loop.} We subject the current unmodified open
  source version of floodlight (git commit f37046a) to fuzz testing with a three
  node fully meshed topology and high failure event rates. In hour experiment,
  After 49 full iterations, the fuzzer found an event sequence with 777 total events
  (548 input, 229 internal) that results in a 3-node forwarding loop being set up.

  Based on the Java JVM, floodlight makes use multiple kernel level
  multithreading, and thus can exhibit non-deterministic behavior in the process.
  Thus, it is not surprising that we do not achieve full reproducability of this
  bug during replay without further instrumention. On average, 15/50 (30\%) of
  replays reproduce the bug. To proceed with MCS isolation, we set the number of
  verification runs for unsuccessful replays to 13. Statistically, this enables
  STS to correctly diagnose violations in >99\% of cases\footnote{$ln( 1 -
  0.99) / ln( 1 - 0.30) = 12.9$}.

  In this, the MCS algorithm was able to reduce the number of input event s from
  548 to 404 ($26\%$) in 168 iterations. Comparing output traces of successful and
  unsuccessful runs, we notice that the bug seems to correlate with specific
  ...
  This experiment is provides a baseline for a worst case scenario of our system.
  \projectname~exercises an unmodified, multithreaded controller it has no full
  control over. The bug appears to depend on fine-grained thread-level races
  conditions that are difficult to guarantee. Still, \projectname was instrumental
  in pointing out a previously unknown bug, and reducing the input size (though
  not dramatically), comparisions of failed and successful replays provide intution
  to the source of the problem.\noindent{\bf Floodlight discovery loop.}

\noindent{\bf Overall Results.} The overall results of our case studies are
shown in Table~\ref{tab:case_studies}. For the Replay Success Rate column we
...
POX in-flight blackhole, we were able to eliminate the relevent
non-determinism by employing multiplexed sockets and waiting on
POX's logging messages.

We show the initial input size and MCS input size in the last two columns. We also
show the input sizes excluding dataplane forwarding permit events, since these
inputs are an artefact of how we currently store and replay event traces. We
plan on making dataplane permits a default.
...
which took more than 5 and a half hours. Nonetheless we observe that even long iteration
