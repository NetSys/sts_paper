...
causal sequence \num{in all cases}. We describe these case studies in detail here.

Our investigation involved roughly 5 days of study.

\colin{Move to discussion section?}
The case studies we present here are relatively simplistic. This is because
the available open source controllers are simplistic.
Some production controllers behind closed doors.

Anecdotes: huge amount of complexity behind closed doors. But for a number of
reasons, it's difficult to discuss bugs publicly. Now in process of adopting
the tool rather than extracting bugs from industry.
...
POX migration blackhole & 4 pod (20 switch) FatTree & \num{?} & \num{?} & \num{?} \\
...
\noindent{\bf POX list removal.} The first control platform we examine is
...
\colin{Emphasize that POX is designed for research prototyping and educational
use (\ie~is not focused on production networking). Still, POX has a diverse
and growing set of users.

We illustrate type errors to show how STS is a useful framework for exploring
code paths that you forget to unit tests. Right now, Mininet is the state of
the art for prototying. Mininet is excellent for testing common scenarios on
fully operational switches (OVS). But it's not explicitly designed to
exercise corner-case scenarios such as link failure or host
migrations.(Support was just added recently)}
...
a switch failure followed by a switch recovery. Apparently the developers of
POX had not tested this input sequence before, and \simulator~made it easy to
identify what inputs caused the erroneous code path to be triggered. \eat{ At the least,
...
% A similar bug was found in NOX learning switch application by NICE. We only
% realized this after discovering the bug.

\noindent{\bf POX discovery blackhole.} The next bug we encountered was found in
...
\begin{figure}[t]
    ...
    \caption[]{\label{fig:list_runtime} Minimizing the POX discovery blackhole.}
\end{figure}

\begin{figure}[t]
    %\hspace{-10pt}
    \includegraphics[width=3.25in]{../graphs/runtime/nox_loop.pdf}
    \caption[]{\label{fig:list_runtime} Minimizing the NOX discovery loop.}
\end{figure}
