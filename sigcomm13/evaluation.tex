...
(involving multiple OpenFlow intitialization handshakes), this is not entirely
unsurprising. Consider that the state-of-the-art open source technology for prototyping SDN
...
switch failure/recovery. \projectname~made it very simple to inject inputs
at a high semantic-level, and the minimized event
trace it produced made for a simple and replayable integration test.
...
discovery of link and host locations. We discovered this bug
on a simple topology, depicted in~\ref{fig:simple_topo}, consisting of two hosts A and B
...
There were seven traffic injection input events ($A\rightarrow B$ and $B\rightarrow A$)
At the moment the link is discovered, POX correctly deletes all flow entries,
and unlearns all host locations. Discovers B correctly at switch 2.2.
An in-flight packet from A shows up at switch 2.1, POX incorrectly learns that A
is at switch 2. Believes hosts are on same switch, installs flow from A->B
2.l->2.r. Packet from B was in flight, arrives at switch 1.1. POX incorrectly
learns B at switch 1, installs path 2.1 ->

The POX routing application (`l2\_multi'). We set up a simple two switch
topology with a host attached to either end, and had each switch.
% Discussion of how our gizmos helped.

The behavior of OpenFlow is to drop packets that are destined out their out
port even if.

Not found in Mininet because...

% ---

Oscillating for a bit. But this isn't the important part.
