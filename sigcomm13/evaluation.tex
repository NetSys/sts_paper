...
POX~\cite{pox}, NOX~\cite{nox}, and Floodlight~\cite{bigswitch}, and
found \num{several} bugs in total. We describe them in detail here.

\noindent{\bf POX empty list removal.} We start with bugs found in POX~\cite{pox}. POX,
the sucessor of NOX, is a single-machine control platform designed in the spirit of
Onix~\cite{onix} to provide programming
abstractions to hide the low-level details of communicating with switches.
...
\item How often does ambiguous fingerprinting occur in practice?
\end{itemize}
...
\begin{figure}[t]
    ...
    \caption[]{\label{fig:hsa_runtime} Minimizing the POX list remove exception.}
\end{figure}
