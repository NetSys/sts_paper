...
POX migration blackhole & 4 pod (20 switch) FatTree & [20/20*] & \num{?} & \num{?} \\
POX discovery blackhole & 2 switch mesh & 15/20 [20/20*] & 68 (26) & 25 (11) \\
...
Floodlight loop & 3 switch mesh & \num{?} & \num{?} & \num{?} \\
% Andi
\end{tabular}
\caption{Overview of Case Studies. Totals shown in parentheses are with
dataplane permit events excluded. \newline
\textmd{*with multiplexed sockets and
logging interposition enabled.}}
\label{tab:case_studies}
\end{table*}

We have applied \projectname~to three open source SDN control platforms:
POX~\cite{pox}, NOX~\cite{nox}, and Floodlight~\cite{bigswitch}. Over a span of roughly five days of
...
\noindent{\bf POX.} The first SDN control platform we examined
was POX, the successor of NOX. POX
...
communication. It is intended primarily for research prototyping and
educational use (\ie~not large scale production use).
Nevertheless, POX has been deployed on real networks, and has a growing set of users.

The POX application we ran was a layer two routing module (`l2\_multi') that
learns host locations and installs per-flow paths between known hosts using a variant of the
...
\noindent{\bf POX list removal.} We start with a relatively trivial bug to illustrate that
...
(which involved multiple OpenFlow intitialization handshakes), this is not entirely
unsurprising.
...
\projectname~made it very straightforward to inject inputs
at a high semantic level (in this case, randomly generated without human
intervention), and the minimized event
...
\noindent{\bf POX discovery blackhole.}
...
on a simple topology, depicted in Figure~\ref{fig:simple_topo}, consisting of two hosts A and B
...
There were 68 inputs in the initial trace, and \simulator~returned a 25 input
MCS. With the MCS in hand we took out paper and pencil to decipher what had
transpired.

Concurrent with the discovery module sending LLDP packets to learn links
there were six traffic injection input events between hosts ($A\rightarrow B$ and $B\rightarrow A$)
with intermediate dataplane permit packet input events at each switch.
POX was learning the locations of the hosts in the following pattern: host $A$
(for example) would send a packet, $S1$ would notify the controller of a new
flow that it did not have a routing entry for, POX would learn the location of
$A$ at port $1.2$, would flood the packet for the unknown destination $B$, the
packet would arrive at $2.1$, $S2$ would notify POX of the unknown flow,
and POX would incorrectly learn a new location
for $A$ at port $2.1$.\footnote{This seems to be necessary behavior, since
host $A$ could have legitimately migrated to $S2$.}
This did not appear to be problematic though,
since we observed that POX flushed the routing entries of the
switches and flushed its incorrect learned location of $A$ at $2.1$ when it discovered
the link between $S1$ and $S2$. At this point POX's only knowledge was the
correct location of $B$ at $2.2$.

After POX discovered both ends of the link, an {\em
in-flight} packet from $A\rightarrow B$ arrives at port $2.1$, and POX incorrectly
learns $A$'s location at $S2$ (this time without a prior notification from
$S1$ about the $A\rightarrow B$ flow).
POX now believed that $A$ and $B$ were attached to the
same switch, and proceeded to install a single flow entry (which is
incidentally correct) in $S2$ from $2.1$ to $2.2$ for $A$'s flow to $B$.

Next, another {\em in-flight} packet from $B\rightarrow A$ arrived at port
$1.1$, $S1$ notified POX of the unknown flow, and POX incorrectly learned a
new location for $B$ at $1.1$. At this point it attempted to set up a route
from $B$ to $A$. However, it incorrectly believed at the time that $B$ was located
at $1.1$ and that $A$ was located at $2.1$, so it installed route entries for
$B\rightarrow A$ out port $1.1$ and out port $2.2$.

% Something weird: A->B doesn't have an entry in S1. So, when a sends a packet
% next, shouldn't this solve the routing loop? There needs to be 4 entries for
% there to be a permanent loop.
At this point no further packets were in flight. Since the switches now had
flow entries for all pairs, no further flow notification messages were sent to
POX, so POX never learned the correct locations of $A$ and $B$.
The behavior of OpenFlow is to drop packets match flow entries that put them
out the port they came in on (unless explicitly told to forward in such cases), hence the
blackhole we observed: any traffic from $B$ would arrive at $S1$ and be
...
\noindent{\bf POX migration blackhole.}
Does learning on new host locations. And installs routes for new flows. But
blackholes pre-existing routes.
We contacted the lead developer of POX, and discovered that this was a
known problem.\footnote{We also noticed after encountering the bug that
NICE~\cite{nice} had found a similar bug in the NOX learning switch
application}


\noindent{\bf NOX discovery loop.}

Loops in SDN have more dire implications than loops in traditional networking.
This is because transient loops in fully-distributed normally resolve
themselves quickly. Loops caused by reactive SDN controllers, however,
persist for as long as the flow mods timers. This is because the SDN
controllers depend on PacketIn's to update their view of the network, yet
packets in a loop cause no flow mods to be sent.

\noindent{\bf Floodlight discovery loop.}
Open-source version of Floodlight provides minimal support for distribution.
This is why we didn't try to reproduce, \eg~the distribution bug. We expect
that our technique would truly shine in those cases.

\colin{Should have a discussion of what caused the non-determinism we
observed, and how it can be mitigated. i.e., how effective gizmos were.}
...
Figure~\ref{fig:list_runtime}.


\begin{figure}[t]
    %\hspace{-10pt}
    \includegraphics[width=3.25in]{../graphs/runtime/pox_blackhole.pdf}
    \caption[]{\label{fig:pox_discovery} Minimizing the POX discovery blackhole.}
\end{figure}
...
\item Overhead: none! Logs already there. Deterministic replay on the other
