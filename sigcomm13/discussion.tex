...
\noindent{\bf What if multiple internal events
are in the same equivalence class as a single event in the original trace?}
Great question. \colin{Pending Sam's formalism}
...
works out of the box. If the controller happens to implement
virtualization (and maintains views in a routing table-like datastructure),
correspondence checking can be used to check invariants, and requires only a small amount of code
to extract the polices from the virtual view.
