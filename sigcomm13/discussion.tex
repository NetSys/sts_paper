...
Maybe not, but since we built the simulator
in software, the fidelity of our simulated failures
can be arbitrarily sophisticated.\footnote{Subject to scalability
...
\noindent{\bf Why SDN?}
SDN has two properties that make it particularly amenable to an
...
topology changes occur for a period of time. This means that
most inputs are not relevant to triggering a given bug, since the system
repeatedly returns to the same quiescent state.
If this were not the case, our
technique would not be able to significantly decrease the size of execution traces.

SDN's architecture also facilitates the implementation of~\projectname.
...
\noindent{\bf Will SDN control software ever become stable enough that
