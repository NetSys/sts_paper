...
\Simulator~leaves open several questions, which we discuss here.
\colin{Should add a discussion of non-determinism within individual
controllers}

\noindent{\bf Aren't the bugs described here fairly simple?}
This is a bit of a chicken and the egg problem:
there are only handful of deployed SDN control platforms, and these are behind
closed doors.

In part, this because the available open source controllers themselves
are simplistic. The open source version of Floodlight, for example, 
Some production controllers behind closed doors.

Moreover, the bugs we encountered were found in roughly five days of
investigation. We 
we expect that we would find 
...
\noindent{\bf Are simulated failures really indistinguishable
from actual failures?}
There will always be some failure modes observed in practice that
are not reproducible without adding significant complexity to the simulator.
For example, suppose that the controller's faulty behavior is to flood
switches with messages, occasionally causing them to drop some traffic, but only when they
are running low on memory.

Luckily the simulator is built in software, and can be modified to model a
wide range of failure modes (subject to scalability
limitations of fine-grained simulations).
As new failure modes are observed in practice, they can be added to the model
if it is deemed worthwhile to do so.
...
\noindent{\bf Why do you focus on SDN networks?}
SDN represents both an opportunity and a challenge. In terms of a challenge,
SDN control platforms are in their infancy, which means that they have bugs
that need to be found and corrected.  Based on our conversations with
commercial SDN developers, the need for automated troubleshooting for SDN
networks is extreme. In terms of an opportunity, SDN control platforms have
two properties that make them particularly amenable to an automated
troubleshooting approach such as ours. First, and most importantly, SDN
control software quickly
converges to quiescence–that is, SDN controllers become idle when no policy or
topology changes occur for a period of time. This means that most inputs are
not relevant to triggering a given bug, since the system repeatedly returns to
a valid quiescent state; one need only study the critical transition from the last
quiescent valid configuration to the first invalid configuration to find the
triggering inputs. If this were not the case, it is not clear that the minimal
causal subsequences found by our technique would be small, in which case
our approach would not yield significant advantages.

Second, SDN's architecture facilitates the implementation of~\projectname.
The syntax and semantics of interfaces between components of the system (\eg~OpenFlow
...
well-defined--a property that is crucial for fingerprinting.
Moreover, controllers are small in number compared to the size of
