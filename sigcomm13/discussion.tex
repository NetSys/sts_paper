...
topology changes occur for a period of time. In contrast to long-running
iterative computations \scott{explain},
...
that the new outcome will be radically different from the original
\scott{Rephrase: many inputs are unimportant, but some are important}. If this were not the case, our
technique would not be able to significantly decrease the size of execution traces, as will will become clear in
\S\ref{subsec:algorithm}. \scott{Term: short causal chains}
