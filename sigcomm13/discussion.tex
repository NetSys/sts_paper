...
\noindent{\bf Aren't the bugs described here fairly simple?} It is true that the
applications we investigate and the bugs we describe are relatively simplistic.
Even these simple applications are obviously difficult to get right: We were
surprised how quickly our tool was able to identify policy violations in the
standard routing modules of \emph{all} investigated platforms, mainly because of
unexpected interactions of different elements in the control plane (e.g.,
shortest path routing and topology discovery). We expect more complex bugs to
surface once we aim our tool at more complex platforms.

\noindent{\bf Aren't SDN controllers relatively simple and bug free?}

In a short period of time, we were able to demonstrate bugs in all investigated
freely available SDN applications. In comparision with these simple open source
platforms and applications, production SDN platforms face significant additional
challenges: Larger deployments cannot be managed with reactive micro flows, and
...
their north- and southbound interfaces. From our conversations with several SDN
controller vendors, we are aware that they all invest significant resources to
deal with these complexity issues. Several commercial players have voiced
interest in our tool. We intend to run our tool (and expect it to be primarily
...
approac is not particularily well-suited to model fine-grained low-level
behavior, especially on the data plane, e.g., when switches are dropping
packets due to memory or slow path contraints. Conversely, it excels at
investigating corner cases in distributed control plane interactions, which
are the source many complex bugs. That said, as the system is entirely
...
controller platforms were exercises with\ \Simulator~without any modifications.
Limited changes to the controller platforms (e.g., the possibility to fix random
seed values, overwriting gettimeofday()) can increase replay accuracy further.
In general, we expect \simulator~ to support controllers conforming to the
...
topology changes occur for a period of time\footnote{Complex bugs occur when
