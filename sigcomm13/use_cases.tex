...
design. Furthermore, we describe three ways developers and operators can put
\projectname~to use.

We show the input types supported by \projectname~in Table~\ref{tab:inputs}.
We notify controllers about link, switch, and host migration events
by sending OpenFlow messages~\cite{openflow} from software switches.
...
statement, notify \projectname~that a new state transition
...
currently manually replace any such functionality with deterministic
algorithms. Our current implementation does not account other sources of non-determinism,
such as asynchronous signals,
or interruptable instructions (\eg~x86's {\tt ecx})~\cite{Dunlap:2002:REI:844128.844148}.

Developers and operators can use \projectname~in a number of ways. We outline three use-cases here.
\colin{Present these as a workflow instead. Emphasize MCS more. Separate
concerns: bug exploration, then replay, then MCS}
...
likely that the same underlying bug is responsible.

\subsection{Replayable Traces}

\andi{Underwhelming. Also, cite OFR}
...
With \projectname, developers can record errant executions in the simulated
environment, and exchange replayable traces to be re-simulated again at other
developer's machines. Replayable traces shorten the debugging cycle, and increase chances that others
will be able to help. For example, developers can file bug reports with a replayable trace
attached, so that no
...
\andi{See if we can shorten this}
We have implemented and exercised the above two use cases.
Forensic analysis of production logs, while not currently implemented,
may be more valuable yet: correctness
violations encountered in production are
high priority and often time consuming to diagnose.
Here we present a sketch of
...
from the original input event. There will of course be some input events that
...
events guarantees that redundant events do not appear. Alternatively, one
...
snapshots.\footnote{Technically the MCS may contain inputs from the
