...
and used by \colin{N} software development teams outside of our own.

In the rest of this section, we highlight salient points of \projectname's
design. Furthermore, we describe three ways \projectname~can be used
...
Although the software switches support packet forwarding, we
have chosen to deemphasize dataplane performance in favor of
being able to simulate networks of very large sizes. % as we show in \S\ref{subsec:runtime}.
...
are received (and consumed) by the controller(s), we \colin{TBD}.

So long as the controller under test supports OpenFlow, \projectname~requires
no modifications to the control software. That said, we make several optional
modifications to further mitigate non-determinism when appropriate. Interposing on
the controllers' logging library allows us to notify \projectname~whenever a log statement is executed
so that \projectname~can account for relevant internal state changes while scheduling inputs.
...
has testified to the value of using Google's network simulator to
fine-tune the network before pushing to production~\cite{vadhat}:
\begin{quote}
...
totally-ordered log of the events in the system to be replayed later.

As additional failure cases are encountered, new cases can be added
to a suite of integration tests. We envision a repository of test
cases being used to validate the correct behavior of other SDN controllers, as
well as future versions of the same software.

Automatically generating integration tests and identifying minimal causal sets
of the inputs for failing tests frees developers to be more agile in
development, spending less time writing test cases, doing triage and bug hunting.

\colin{Mozilla redundant bug pitch?}
