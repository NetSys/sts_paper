...
implemented this functionality, we present a sketch for
...
necessitating some instrumentation and preprocessing before \simulator~can be
invoked.

Production system would need to include Lamport
clocks on each message~\cite{Lamport:1978:TCO:359545.359563} or have
sufficiently accurate clock
synchronization~\cite{corbett2012spanner} to obtain a global ordering
consistent with the happens-before relation.
Note that it is permissible for \simulator~to reorder concurrent events from
the production run so long as the happens-before relation is
maintained~\cite{tel2000introduction}.
...
That said, the fidelity of the software simulation is bounded only by the time invested to model
the subtleties of failure modes.
...
errant configurations by examining that state with a debugger}
