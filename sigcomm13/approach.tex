To illustrate the mechanics of \simulator, we start by describing an example bug in the Floodlight
...
\textepsilon~should be set to. We experimentally varied \textepsilon on two of the bugs in \S\ref{sec:evaluation},
and found that the numbers of events \simulator~timed out on when became stable for values above $25$ milliseconds.
When \textepsilon is set to $0$ milliseconds, the number of events timed out drops rapidly for small values. 
After the latency threshold around $25$ milliseconds is crossed, no significant gains in percentage of events
matched are achieved by increasing \textepsilon.
...
Analysis of event frequencies from traces could
more precisely provide ideal \textepsilon values for particular bugs or controller configurations
