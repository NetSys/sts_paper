...
In particular, our problem differs in two dimensions. First, input to SDN controllers is spread
...
\begin{figure}[t]
    %\hspace{-10pt}
    \includegraphics[width=3.25in]{../diagrams/state_machines/controller.jpg}
    \includegraphics[width=3.25in]{../diagrams/state_machines/switch.jpg}
    \caption[]{\label{fig:architecture} State machines for the switch and
    controller in the Floodlight bug}
\end{figure}
...
successor occuring before it has injected the next input.\colin{Should
formally describe a hybrid algorithm--only invoke peek() for one interval, and
only when a successor has been observed}
\colin{Mention that causal annotations might help}
