...
a minimal set of fault-inducing inputs from logs. Before diving into the
...
how the control software reacts to the occurrence of (i) and/or (ii).
...
our approach easily supports deterministic replay.

\noindent{\bf Globally vs Locally Minimal Input Sequences.}
Delta debugging is not guaranteed to find the globally minimum
...
We are primarily focused on correctness bugs, not performance bugs. That said, this
 distinction primarily relates to invariant checking, and not to the
 task of finding an minimal causal set.
...
open source control platform~\cite{floodlight_bug}, and later show how our
technique is applied to this bug. Floodlight is distributed across
...
to a distributed setting; our work demonstrates that
delta debugging can be used to localize fault-inducing inputs from execution traces of
SDN controllers.

In particular, our problem differs in two dimensions. First, the input to SDN controllers is spread
...
In the rest of this section, we describe how we (i)
coordinate replay across the distributed nodes in SDN networks,
(ii) cope with alterations to the causal history of an execution, and (iii)
develop a generic invariant checking algorithm
for detecting errant behavior. In \S\ref{sec:architecture}, we describe concrete ways in
...
or policy changes) chosen by the delta debugging algorithm\footnote{We discuss
how the original sequence of inputs is obtained in \S~\ref{sec:architecture}}, the
simulator replays these inputs to the control software. For example, link failures are
reproduced by disconnecting the edge in the simulated network, and sending a
port status message from the adjacent switches to their parent controllers.

The relative timing of the inputs injected by the simulator is crucial for ensuring that the
...
the timestamp of the input from the original run. In other words, simply
...
While the input events are given to us, we become aware of internal events by
...
control message receipts both between controllers and between switches and
...
tool. To be completely deterministic, these log statements would need to
include information about every thread scheduling decision
and every instruction executed; we show in \S\ref{subsec:case_studies}
...
Unfortunately, event scheduling is made substantially more complicated by the fact
...
semantically extraneous fields of the message. A few examples of masked fields are shown
...
happens-before relation. \colin{Pending Sam's formalism}
...
The second implication of altering the original trace is that
...
We have developed two event scheduling algorithms to cope with this
...
Simply put, it wait for internal events,
but times out and proceeds if they do not show up.
...
for e\textsubscript{i} in trace:
  if e\textsubscript{i} is an internal event:
    \textDelta = |time(e\textsubscript{i}) - time(e\textsubscript{i-1})| + \textepsilon
...
Our second algorithm addresses this issue, but
adds a factor of $N$ in the number of inputs to the overall runtime. We refer to it as \emph{peeking
replay}, and it is shown in Figure~\ref{fig:peek}.
...
for a discussion.
\item In~\S\ref{sec:architecture}, we discuss some of the implementation details of handling internal events
\end{itemize}
...
While our approach supports generic deciders (\eg~all-pairs reachability or loop detection),
we show here that with a small modification to the control software we can
