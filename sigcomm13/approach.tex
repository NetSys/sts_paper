...
ways developers and operators can put \simulator~to use.
...
Note that we could convert the extra factor of
$n$ in computational complexity due to backing up to a factor of $n$ in storage
complexity if we instead stored snapshots of
the controllers' state rather than replaying each prefix from the beginning.

\colin{Mention parallelization. Each DD subset is independent!}
...
The delta debugging algorithm we employ however provably finds a
