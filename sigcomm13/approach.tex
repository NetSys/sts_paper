...
simulation process whenever log statement (which serve to mark relevent state
...
\begin{figure}[t]
    ...
    respresent correctness violations--a blackhole in this case. \colin{TODO: broadcast
    notification?}}
\end{figure}
...
A more problematic implication of altering the original trace is that
...
switch will not make the internal state transition from `Safe` to
`Notify Master` as it did in the original execution.
...
injected. For example, if $i_2$ and $i_3$ are internal events observed
...
this implies an exponential number of possibilies to be explored during
replay.
...
occurs when a subpath exists in the distributed state machine that contains two or
...
arbitrarily to $100$ milliseconds, and this has worked for \colin{most} of our
...
has occured), but we cannot detect when we have timed out
