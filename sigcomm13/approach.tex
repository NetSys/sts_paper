...
\andi{Don't reference production datacenter logs here. Otherwise we'll get the 
``Ha-Ha---you can't actually instrument a datacenter`` reaction}
...
\andi{Section is way to long. Split in~,e.g., 1) Architecture 2) Event Scheduling
3) Dealing with Divergence 4) Finger-printing 5) Computational Complexity. Emphasize that dealing with divergence
is important, and only tractable b/c of domain knowledge}
...
non-deterministic behavior. We therefore simulate the entire dataplane \andi{Saying dataplane here is problematic
because it sweats scalability problems. Say we simulate the control-plane behavior of the network?}
, including hosts and
...
potentially resulting in an altered output. \sam{Is it worth noting that this isn't necessarily a problem? If a successor occurs, it means it wasn't causally related to $e_i$, but the ordering still might matter for further successors.}
...
provided by frameworks such as Hassel~\cite{hsa} or Anteater~\cite{anteater}.
