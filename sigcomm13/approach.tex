...
example, it is not sensible to replay a link recovery event without a
preceding link failure event; nor is it sensible to replay a host migration
...
paper~\cite{Zeller:2002:SIF:506201.506206} that removes the need for the assumption,
but the algorithmic complexity increases by a factor of $N$ in doing so.}
...
after an input that preceded it in the original run has been injected.

The challenge is to ensure that causal relationships
...
occurred~\cite{Fischer:1985:IDC:3149.214121}.
...
\subsection{Coping With Absent Internal Events}
...
replays the trace from the beginning up until the immediately prior input, this time
knowing exactly which internal events in the current input interval are
and are not going to occur before injecting the next input.\footnote{An alternative
would be to take a snapshot of the controllers' state at every injected input
and start from the latest snapshot} The overall event scheduling algorithm
is shown in Figure~\ref{fig:peek}.

\begin{figure}
\begin{boxedminipage}{\linewidth}
\begin{Verbatim}[commandchars=\\\{\}]
trace = [e\textsubscript{1}, e\textsubscript{2}, ..., e\textsubscript{j}]
  ...
  for e\textsubscript{i} in trace:
    if e\textsubscript{i} is an internal event and
      ...
      if a successor of e\textsubscript{i} occurred:
        // waited too long
        return replay(epsilon, trace)
      else:
        inject e\textsubscript{i}
\end{Verbatim}
\end{boxedminipage}
\caption{Replay Algorithm Pseudocode.\colin{Not actually correct! Waiting or not
waiting potentially induces changes. Also,
potentially recurses infinitely if not deterministic}}
\label{fig:peek}
\end{figure}

\subsection{Coping With New Internal Events}
...
It is theoretically always possible to construct two state machines that lead
...
possibilities for every new internal event. In the worst case,
this implies an exponential number of possibilities to be explored during
replay.

Luckily, crucially ambiguous new internal events are \num{extremely rare} in the
...
always injecting the next input when the last expected predecessor
...
We conjecture that this is because SDN is a control plane system,
...
If the event scheduling algorithm
...
has occurred) \sam{Maybe move this statement in parens earlier, like 3.6. When we say ``we can detect when we have waited too long'', we should state how we know this.}, but we cannot detect when we have timed out
...
practice most relevant internal state changes are already marked by logging
