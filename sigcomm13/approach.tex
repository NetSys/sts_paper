...
the  distinction primarily relates to invariant checking, and not to the
 task of finding a minimal causal set.
...
We have developed two event scheduling algorithms, each with increasing
...
errors, we switch over to peeking replay only when waiting replay is
insufficient.
...
cases. In the best case, the delta debugging algorithm terminates after $O(log n)$
...
if peek stores snapshots of
the controllers' state rather than replaying each prefix from the beginning.
\colin{Forward pointer to what we do about multiple matching fingerprints?}
\colin{Optimization: Pruning types of inputs}

We make another optimization to delta debugging by applying domain knowledge
...
Note that beyond component failures and host migration, other dependencies between
