...
necessary for triggering errant behavior. We define errant behavior in terms of {\em correctness violations}:
...
correctness violation was originally discovered. Policy-violations
...
Therefore, it is sufficient to use a `blunt hammer' (\eg{} POSIX's $SIGKILL$) to reproduce individual
...
\colin{TODO: discuss additional benefits of a deterministic simulator.
Integration testing, etc. Use Amin's quote as motivation for the software
simulator.} 

\colin{Specific benefits of simulator:
\begin{enumerate}
\item lifetime tracking
\item if can't reproduce failure from production, fuzz
\item discover bugs by periodically invoking corresponsdence checking. (how
frequently?)
\item integration test library
